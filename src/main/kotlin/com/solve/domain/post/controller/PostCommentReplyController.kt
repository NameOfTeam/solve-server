...
class PostCommentReplyController(
    ...
    fun createReply(
        @PathVariable postId: Long,
        @PathVariable commentId: Long,
        @RequestBody request: PostCommentReplyCreateRequest
    ) = BaseResponse.of(postCommentReplyService.createReply(postId, commentId, request))
    ...
    fun updateReply(
        ...
        @RequestBody request: PostCommentReplyUpdateRequest
    ) = BaseResponse.of(postCommentReplyService.updateReply(postId, commentId, replyId, request))

    @Operation(summary = "게시글 댓글 답글 삭제")
    @DeleteMapping("/{replyId}")
    fun deleteReply(@PathVariable postId: Long, @PathVariable commentId: Long, @PathVariable replyId: Long) =
        BaseResponse.of(postCommentReplyService.deleteReply(postId, commentId, replyId))
}