...
class PostCommentLikeController(
    ...
    @PostMapping
    fun likeComment(@PathVariable postId: Long, @PathVariable commentId: Long) =
        BaseResponse.of(postCommentLikeService.likeComment(postId, commentId), 201)

    @Operation(summary = "게시글 댓글 좋아요 취소")
    @DeleteMapping
    fun unlikeComment(@PathVariable postId: Long, @PathVariable commentId: Long) =
        BaseResponse.of(postCommentLikeService.unlikeComment(postId, commentId), 204)
}